...
open class Mock: DefaultProvider, @unchecked Sendable {
    ...
    /// Provides a ``Spy`` instance for a given static member of a specific type.
    ///
    /// This method allows composition-based generated mocks to retrieve static spies
    /// without relying on `Self` resolution through inheritance.
    ///
    /// - Parameters:
    ///   - typeName: The string representation of the static mock type.
    ///   - member: The name of the member being accessed.
    /// - Returns: A ``Spy`` instance configured for the member's signature.
    public static func staticSpy<each Input, Eff: Effect, Output>(typeName: String, member: String) -> Spy<repeat each Input, Eff, Output> {
        lock.lock()
        defer { lock.unlock() }
        let provider = MockScope.storageProvider
        var storage = provider.storage[typeName] ?? [:]

        if let spyGroup = storage[member], let existingSpy = spyGroup.firstMap({ $0 as? Spy<repeat each Input, Eff, Output> }) {
            return existingSpy
        } else {
            let methodLabel = "\(typeName).\(member)"
            let spy = Spy<repeat each Input, Eff, Output>(label: methodLabel)
            spy.isLoggingEnabled = isLoggingEnabled
            spy.defaultProviderRegistry = defaultProviderRegistry
            storage[member, default: []].append(spy)
            provider.storage[typeName] = storage
            return spy
        }
    }
...
/// A type that owns a backing ``Mock`` instance which stores its spies.
///
/// Mocks generated by the `@Mockable` macro use composition: they declare a
/// `private let mock = Mock()` and forward spy storage, stubbing and
/// configuration to it. This protocol exposes that backing instance so library
/// helpers (such as ``verifyZeroInteractions(_:)``) keep working with both
/// `Mock` subclasses and composed generated mocks.
public protocol MockBacked: AnyObject {
    /// The backing mock instance that manages spies for this type.
    var mock: Mock { get }
}

extension Mock: MockBacked {
