...
- In a **composed** mock, plain `adapt(...)` doesn't resolve — it's an instance method on `Mock` and you didn't inherit it. Use the static `Mock.adapt(...)` / `Mock.adaptThrowing(...)`. `clear()` *does* resolve: `MockProviding` supplies it, and `StaticMockProviding` supplies the static form for mocks with static requirements.
