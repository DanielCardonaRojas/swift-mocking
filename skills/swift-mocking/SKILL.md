...
- Spy names come from the requirement, ignoring argument labels: methods use their name, subscripts use parameter names in camelCase (`subscript(row:column:)` → `rowColumn`), settable members add `set`+Name. Two requirements deriving the same name *and* signature are rejected at expansion — rename one. Same-name/different-signature overloads are fine.
